              ...
              <div class="panel panel-primary">
				...
				<?php }else{?>
					...
					<div class="col-md-12 form-group">
						<?php echo $this->Form->create('VaData',array('type' => 'get','url' => array('controller' => 'vareports','action' => 'va_reports_list'))); ?>
						...
						<div class="col-md-12 col-sm-12 col-xs-12">
						  <table id="datatable_report1" class="table table-striped table-bordered">
							...
							<tr>
								...
								<td><?php echo (!empty($data['VaData']['eye_select']))?'OD' : 'OS';?></td>
								...
								<td><?php echo ($Admin['user_type'] == "Admin")?  "<a style='cursor: pointer;'  title='View' patientId='".$data['VaData']['patient_id']."'class='patient loaderAjax'>".$data['VaData']['patient_name']."</a>" : "<a style='cursor: pointer;'  title='View' patientId='".$data['VaData']['patient_id']."'class='patient loaderAjax'>".$data['VaData']['patient_name']."</a>"; ?></td>
								<td class="action_sec"> 
									<?php
									...
									<?php
										if(!empty($Admin) && ($Admin['user_type'] == "Subadmin" || $Admin['user_type'] == "Admin")){
											echo "<a href=".$this->Html->url(['controller'=>'vareports','action'=>'delete',$data['VaData']['id']])." title='Delete' onclick='if (confirm(&quot;Are you sure you want to delete?&quot;)) { return true; } return false;'><i class='fa fa-trash-o'></i></a>";
										}
									?>
								</td>
							</tr>
...
jQuery(document).ready(function(){
	...
	jQuery(document).on("click",".testreport",function() {
		...
		jQuery("#reportContent").load("<?php echo WWW_BASE; ?>admin/vareports/view/"+testreportId+ "?" + new Date().getTime()+ new Date().getMilliseconds(), function(result) {
	...
	jQuery(document).on("click",".VaData",function() {
		...
		jQuery("#reportContent").load("<?php echo WWW_BASE; ?>admin/unityreports/view/"+VaDataId+ "?" + new Date().getTime()+ new Date().getMilliseconds(), function(result) {
			jQuery("#reportView").modal("show");
			$('.customFacebox').remove();
		});
	});	
