												...
												<div class="row">
													<div class="col-sm-12">
														<div class="vf-setting-box">
															...
															<!-- Zero cutoff --> <?php //print_r($user_default);die;?>
															<div class="form-group">
																<label style="font-size: 17px;padding-right: 20px;" for="zero_Db_Cutoff">Zero DB Cutoff</label>
																<input type="checkbox" id="zero_Db_Cutoff" class="zeroDb_Cutoff" style="height: 25px;width: 25px;" onchange="zerodbutoff(this)" <?php echo (@$user_default['zeroDbCutoff'])?'checked':'';?>>
															</div>
															<div class="dels-drop">
																<div href="#" class="thrushold">
...
<script>
	...
	jQuery(document).ready(function () {
		...
		$(document).on("change","#progression_analysis",function(){ 
			...
			if(chbox){
				window.sessionStorage.setItem("chbox", chbox);
			}else{
				sessionStorage.removeItem("chbox");
			}
		});
		...
		$(window).load(function () {
			...
			if($_GET['EyeClick'] == "true"){
			?> EyeClick = true;<?php
			}else{
			?> EyeClick = false;<?php
			}
			?>
			$(".eye-click").prop("checked", EyeClick);
			<?php } ?>
	...
	const get_testJsonData = async function () {
		...
		if (view_patient_test_flag_json == 1) {
          ...
          console.log(typeof obj.message[key]['testData']);*/
					...
					EyeClick = d1.StartTest.EyeClick;
	...
	function addProgressionAnalysis(value = false, disable = true) {
     ...
     $.ajax({
      ...
      success: function(data){
		...
		if (<?php echo $flg ?>) {
			if(window.sessionStorage.getItem("chbox")){
				var chbox=window.sessionStorage.getItem("chbox");
				if(chbox){
					value = true;
				}
			}
		...
		if (view_patient_test_flag == 0) {
			...
			if (type == 'testId') {
				...
				if (!saveresult) {
					var r = confirm("Report has not been saved and the last test results will be lost. Do you want to switch the test type?");
					if (r == true) {
						window.location.replace('<?php echo WWW_BASE . 'admin/patients/' . $page_name . '/' . $this->request->params['pass'][0] ?>/' + $("#test-type").val() + '/' + $("#test-strategy").val() + '/' + testTypeName + '?selectEye=' + selectEye + '&GazeTracking=' + GazeTracking + '&EyeClick=' + EyeClick + '&alarm_stop=' + alarm_stop + '&testBothEyes=' + testBothEyes + '&voice_instractuin=' + voice_instractuin + '&progression_analysis=' + progression_analysis + '&bilateralfixation=' + bilateralfixation);
					} else {
						document.getElementById("test-strategy").value = testSubType;
					}
				} else {
					...
					if (enabletestnameliest.includes(type)) {
						if (!saveresult) {
							if ((!dataCaptured) && (!dataCapturedpause)) {
								var r = confirm("Report has not been saved and the last test results will be lost. Do you want to switch the test?");
								if (r == true) {
									window.location.replace('<?php echo WWW_BASE . 'admin/patients/' . $page_name . '/' . $this->request->params['pass'][0] ?>/' + $("#test-type").val() + '/' + $("#test-strategy").val() + '/' + type + '?selectEye=' + selectEye + '&GazeTracking=' + GazeTracking + '&EyeClick=' + EyeClick + '&alarm_stop=' + alarm_stop + '&testBothEyes=' + testBothEyes + '&voice_instractuin=' + voice_instractuin + '&progression_analysis=' + progression_analysis + '&bilateralfixation=' + bilateralfixation);
								}
							}
						} else {
							window.location.replace('<?php echo WWW_BASE . 'admin/patients/' . $page_name . '/' . $this->request->params['pass'][0] ?>/' + $("#test-type").val() + '/' + $("#test-strategy").val() + '/' + type + '?selectEye=' + selectEye + '&GazeTracking=' + GazeTracking + '&EyeClick=' + EyeClick + '&alarm_stop=' + alarm_stop + '&testBothEyes=' + testBothEyes + '&voice_instractuin=' + voice_instractuin + '&progression_analysis=' + progression_analysis + '&bilateralfixation=' + bilateralfixation);
			...
			success: function (data) {
				if (set_user_preset) {
					...
					if (data.user_preset_data.hasOwnProperty('UserPresetData')) {
						...
						EyeClick = (data.user_preset_data.UserPresetData.EyeClick == "true") ? true : false;
		...
		$.ajax({
			...
			success: function (data) {
             ...
             measureReactionTime=false;
								...
								StartTestData.EyeClick = EyeClick;
								if(botheyecount==2){
