  ...
  <div class=""> 
    ...
    <div class="page-header-title">
      <div class="row">
          ...
          <div class="col-sm-2 mts-box-1">
           <?php if (in_array(14, $checked_data)) { echo "<a style='width: 57px; background: #3292e0; color: white; border: 2px solid #f3ecec; margin-top: 3px;'   href='javascript:void(0);' class='btn' title='Start VF Test' >VF</a>"; }?>
                    ...
                    <div href="#" class="thrushold">
                      <div class="dropdown"> 
                         <select class="mmd-dash-btn" id="language"  onChange="updateDevice();" style="height: 30px;">
                            ...
                            <option value="7" <?php echo (!empty($user_default))?($user_default['UserDefault']['language_id'] == 7)?'selected':'':'' ?>>Chinese</option> 
                    ...
                    <div href="#" class="thrushold">
                      <div class="dropdown">
                        <select class="mmd-dash-btn" id="test-type" onchange="reloadPage('testId');" style="height: 30px;">
                          ...
                          <option value="1" <?php echo ($testData['testId']==1)?'selected':'' ?> >Screening</option>
                          <option value="2" <?php echo ($testData['testId']==2)?'selected':'' ?>>Threshold</option>
                          <option value="3" <?php echo ($testData['testId']==3)?'selected':'' ?>>Ptosis</option>
                          <option value="4" <?php echo ($testData['testId']==4)?'selected':'' ?>>Neuro</option>
                          <option value="5" <?php echo ($testData['testId']==5)?'selected':'' ?>>Kinetic/Ptosis</option>
                        </select>
                  ...
                  <div class="col-lg-7 col-sm-8">
                      <div class="mmt-main-canvas" style="position: relative;">
                        ...
                        <span style="font-size: 18px;color: #990000;font-weight: 600; text-align: center;" class="setting-alert-message2"></span>
                         <span style="font-size: 18px;color: #990000;font-weight: 600; text-align: center;" class="setting-alert-message3"></span>
                      </div>
                          <div class="mt-btns gz-trck d-desktop">
                              <div class="mt-checkboxes mt-checkboxes-eye">
                                <div class="form-group"  id="view_patient_test_div">
                                  <input type="checkbox"  id="view_patient_test" class="view_patient_test" aria-label="..." onchange="viewPatientTest(this)">
                                   <label for="view_patient_test"><span type="checkbox" style="font-weight: 500;" id="view_patient_test_txt" title="Viewer Mode allows you to monitor tests started on another browser.">Viewer Mode</span></label> 
                              </div><br><br>
                          ...
                          <div class="border-div d-big-screens" style="">
                              ...
                              <span style="font-size: 16px;color: #990000;font-weight: 600; text-align: center;" class="setting-alert-message2"></span>
                               <span style="font-size: 16px;color: #990000;font-weight: 600; text-align: center;" class="setting-alert-message3"></span>
                            </div>
...
<script> 
  ...
  ["Central_10_2", "Central_24_1", "Central_24_2", "Central_30_1", "Central_30_2"],
 ...
 const get_testJsonData = async function(){  
   ...
   if(view_patient_test_flag_json==1){
          ...
          url: "<?php echo WWW_BASE; ?>css/get_test_json_data.php",
           data: {"office_id": '<?php echo $user['office_id'] ?>', "patient_id": '<?php echo $data['Patient']['id'] ?>' },
          async: false
      }).success(function(){ 
          setTimeout(function(){get_testJsonData();}, 400);
      }).responseText; 
     obj = JSON.parse(feedback);
      for (var key in obj.message) {
        if(obj.message!='No data'){
...
//function setpixcelVF_FILE_UPLOADED(data) {
  if(view_patient_test_flag==1 && testTypeName !='Ptosis_Auto_9_PT'){
   ...
   if(view_patient_test_flag==0){
    ...
    console.log('setpixcelVF_FILE_UPLOADED');
    console.log(testBothEyes);
    console.log(botheyecount);
    console.log(view_patient_test_flag);
    if(botheyecount==1){ 
 ...
 $("#myinput-3").val((value-min)*(100/(max-min)));
  ...
  function updateDevice() {
     ...
     $.ajax({
      ...
      type: 'POST', 
       data: {"page": 1,"deviceId": deviceId,"languageId": language}, 
      success: function(data){ 
        data=JSON.parse(data) 
  ...
  function TestData(type=0) {
    ...
    var language=$("#language").val();
   if(view_patient_test_flag==0){
    console.log('mastewr dataa after view checked');
    $.ajax({
