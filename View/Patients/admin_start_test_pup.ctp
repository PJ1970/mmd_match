           ...
           <?php if (in_array(14, $checked_data)) { echo "<a style='width: 57px; background: #7e7e7e; color: white; border: 2px solid white;  margin-top: 3px;'   href='".WWW_BASE."admin/patients/start_test/".$data['Patient']['id']."'  class='btn' title='Start VF Test' >VF</a>"; }?>
            <?php if (in_array(15, $checked_data)) { echo "<a style='width: 57px; background: #7e7e7e; color: white; border: 2px solid white;  margin-top: 3px;' class='btn'   href='".WWW_BASE."admin/patients/start_test_fdt/".$data['Patient']['id']."' title='Start FDT Test' >FDT</a>"; }?>
               ...
               <?php if (in_array(25, $checked_data)) { echo "<a style='width: 57px; background: #7e7e7e; color: white; border: 2px solid white;  margin-top: 3px;' href='".WWW_BASE."admin/patients/start_test_vs/".$data['Patient']['id']."' title='Start VS Test' class='btn' >VS</a>"; }?>
                <?php if (in_array(34, $checked_data)) { echo "<a style='width: 57px; background: #3292e0; color: white; border: 2px solid #f3ecec; margin-top: 3px;' href='".WWW_BASE."admin/patients/start_test_pup/".$data['Patient']['id']."' title='Start Pupilometer Test' class='btn ' >PUP</a>"; }?>
               </div>
          <div class="col-sm-3 mts-box-1">
                    ...
                    <div href="#" class="thrushold">
                      <div class="dropdown"> 
                         <select class="mmd-dash-btn" id="test-type" onchange="reloadPage('testId');" style="height: 30px;">
                          ...
                          <option value="PUP 1" <?php echo ($testData['testId']=='PUP 1')?'selected':'' ?> >PUP 1</option>
                          <option value="PUP 2" <?php echo ($testData['testId']=='PUP 2')?'selected':'' ?>>PUP 2</option>
...
<script> 
  ...
  var testTypeName='<?php echo (isset($testData['testId']))?$testData['testId']:'PUP 1'; ?>';
  var testSubType=0;
  var selectEye=1;
  var eye_taped=false;
  var stimulusSize=3;
   ...
   $('#start').on('click',function(){ 
      ...
      if ( (restartReady) && deviceId!='' && lang!=0){
        ...
        clearOldMessage();
            ...
            setting_alert_new('The Device is not allowed for PUP test.');
         }
         }else{
        if(deviceId==''){
          alert('Please select device');
    ...
    if(fdt_test_name=='FDT'){
      ...
      success: function(data){
        ...
        var arr=data.VfMasterdata;  
          ...
          StartTestData.Test_Name=testTypeName;
...
function reloadPage(type) { 
  if(type=='testId'){ 
      if (((dataCaptured)|| (dataCapturedpause)|| (!saveresult)) && (testType != -1)){
           ...
           if (r == true) {
              window.location.replace('<?php echo WWW_BASE.'admin/patients/'.$page_name.'/'.$this->request->params['pass'][0] ?>/'+$("#test-type").val());
          } else {
