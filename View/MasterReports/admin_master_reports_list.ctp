...
<div class="content">
      <div class="">
        <div class="page-header-title">
          <h4 class="page-title">Master Test Report</h4>
        </div>
            ...
            <div class="col-md-12">
              <div class="panel panel-primary">
				...
				<div class="col-md-12 form-group">
					<?php echo $this->Form->create('Masterdata',array('type' => 'get','url' => array('controller' => 'masterReports','action' => 'master_reports_list'))); ?>
					...
					<div align="right" class="col-md-4">
							<h4 class="m-b-30 m-t-0">  <a href="<?php echo $this->HTML->url('/admin/masterReports/sendNotification'); ?>" class="btn btn-large btn-primary" >Send Notification</a>  
                      ...
                      <table id="datatable_report1" class="table table-striped table-bordered">
                        <thead>
                          <tr>
                            ...
                            <th> <?php echo $this->Paginator->sort('Masterdata.created','Date'); ?> </th>
                            <th>  <?php echo $this->Paginator->sort('Masterdata.staff_name','Staff User'); ?> </th>
							<th>  <?php echo $this->Paginator->sort('Masterdata.eye_select','OD/OS'); ?> </th>
                            <th> <?php
							echo ($Admin['user_type'] == "Admin")? 'Patient' : $this->Paginator->sort('Masterdata.patient_name',' Patient Name'); ?></th>
                            <th>Action</th>
                          </tr>
                        </thead>
						<tbody>
						...
						<tr>
							<td data-order="<?php echo $data['Masterdata']['id']; ?>"><?php echo $key+1; ?></td>
							<td><?php echo date('d M Y h:i:s a',strtotime($data['Masterdata']['created'])); ?></td>
						 
							<td><?php
							$string = @explode(' ',@$data['Masterdata']['patient_name']);
							$patient_new_id = @substr(@$string[0],0,1).@substr(@$string[1],0,1).$data['Masterdata']['patient_id'];
							echo   "<a style='cursor: pointer;'  title='View' staffId='".$data['Masterdata']['staff_id']."'class='staff loaderAjax' >". $data['Masterdata']['staff_name']."</a>"; ?></td>
							
							<td><?php echo (!empty($data['Masterdata']['eye_select']))?'OD' : 'OS';?></td>
							
							<!--<td><?php echo ($Admin['user_type'] == "Admin")?  "<a style='cursor: pointer;'  title='View' patientId='".$data['Masterdata']['patient_id']."'class='patient loaderAjax'>".$patient_new_id."</a>" : "<a style='cursor: pointer;'  title='View' patientId='".$data['Masterdata']['patient_id']."'class='patient loaderAjax'>".$data['Masterdata']['patient_name']."</a>"; ?></td>-->
							
							<td><?php echo ($Admin['user_type'] == "Admin")?  "<a style='cursor: pointer;'  title='View' patientId='".$data['Masterdata']['patient_id']."'class='patient loaderAjax'>".$data['Masterdata']['test_name'].' , '.$data['Masterdata']['age_group']."</a>" : "<a style='cursor: pointer;'  title='View' patientId='".$data['Masterdata']['patient_id']."'class='patient loaderAjax'>".$data['Masterdata']['test_name'].' , '.$data['Masterdata']['age_group']."</a>"; ?></td>
							
							<td class="action_sec"> 
							<?php
							echo "<a style='cursor: pointer;'  title='View' testreportId='".$data['Masterdata']['id']."' class='testreport loaderAjax' ><i class='fa fa-eye' aria-hidden='true'></i></a>";
							?>
							&nbsp;&nbsp;&nbsp;
							<?php
							echo "<a href=".$this->Html->url(['controller'=>'masterReports','action'=>'delete',$data['Masterdata']['id']])." title='Delete' onclick='if (confirm(&quot;Are you sure you want to delete?&quot;)) { return true; } return false;'><i class='fa fa-trash-o'></i></a>";
							?>
							
							</td>
						</tr>
						<?php }
						  if(isset($this->params['paging']['Masterdata']['pageCount'])){ ?>
							<tr> 
...
jQuery(document).ready(function(){
	...
	jQuery(document).on("click",".testreport",function() {
		...
		jQuery("#reportContent").load("<?php echo WWW_BASE; ?>admin/masterReports/view/"+testreportId+ "?" + new Date().getTime()+ new Date().getMilliseconds(), function(result) {
	...
	jQuery(document).on("click",".Masterdata",function() {
		var MasterdataId = jQuery(this).attr("MasterdataId");
		jQuery("#reportContent").load("<?php echo WWW_BASE; ?>admin/masterReports/view/"+MasterdataId+ "?" + new Date().getTime()+ new Date().getMilliseconds(), function(result) {
			jQuery("#reportView").modal("show");
			$('.customFacebox').remove();
		});
	});	
