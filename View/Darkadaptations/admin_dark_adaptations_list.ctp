...
<div class="content">
      <div class="">
        <div class="page-header-title">
          <h4 class="page-title">DA Test Report</h4>
        </div>
					...
					<?php
						...
						<?php $Admin = $this->Session->read('Auth.Admin');?>
							...
							<div class="col-md-12 form-group">
								<?php echo $this->Form->create('DarkAdaption',array('type' => 'get','url' => array('controller' => 'darkadaptations','action' => 'dark_adaptations_list'))); ?>
						  ...
						  <table id="datatable_report1" class="table table-striped table-bordered">
							<thead>
							  <tr>
								...
								<th> <?php echo $this->Paginator->sort('DarkAdaption.test_date_time','Test Date'); ?> </th>
										<th>  <?php echo $this->Paginator->sort('DarkAdaption.patient_name','Patient name'); ?> </th>
								<th>  <?php echo $this->Paginator->sort('Patient.id_number','Patient id'); ?> </th>

								<th>DOB  <br>(DD-MM-YYYY)</th>

								<th>  <?php echo $this->Paginator->sort('DarkAdaption.eye_select','OD/OS'); ?> </th>
							 <?php if($Admin['user_type']=='Admin'){ ?>	<th>Office Name</th>  <?php } ?>
								<th>  <?php echo $this->Paginator->sort('DarkAdaption.staff_name','Staff User'); ?> </th>
							...
							<tr>
								...
								<td>
									...
									<a style='cursor: pointer;'  title='View' patientId='<?php echo @$data['DarkAdaption']['patient_id'] ?>'class='patient loaderAjax'><?php echo @$data['Patient']['first_name'].' '.@$data['Patient']['middle_name'].' '.@$data['Patient']['last_name']; ?></a>
								</td>
								...
								<td>
									<a style='cursor: pointer;'  title='View' staffId='<?php echo @$data['DarkAdaption']['staff_id'] ?>' class='staff loaderAjax' ><?php echo @$data['User']['first_name'].' '.@$data['User']['middle_name'].' '.@$data['User']['last_name']; ?></a>
								</td>
								<td class="action_sec">
									 <?php
									...
									<?php
										echo "<a style='cursor: pointer;'  title='View' testreportId='".$data['DarkAdaption']['id']."' class='testreport loaderAjax' ><i class='fa fa-eye' aria-hidden='true'></i></a>";
									?>
									...
									<a data-type="dicom" data-related='<?php echo $related_id; ?>' data-downloads='<?php echo json_encode($download[$ptdata_id]); ?>' title='Download OS/OD Dicom Reports' href="javascript:;" class="vbs-popover" ><i class="fa fa-download"></i></a>
									 <a title="Download Dicom Report" href="<?php echo $this->Html->url(['controller'=>'unityreports','action'=>'exportDicomda',$data['DarkAdaption']['patient_id'], $data['DarkAdaption']['pdf']]); ?>" target="_blank"><i class="fa fa-image" aria-hidden="true"></i></a>
									&nbsp;&nbsp;&nbsp;
									<?php //pr($Admin); die;
										if(!empty($Admin) && ($Admin['user_type'] == "Subadmin" || $Admin['user_type'] == "Admin")){
											echo "<a href=".$this->Html->url(['controller'=>'darkadaptations','action'=>'delete',$data['DarkAdaption']['id']])." title='Delete' onclick='if (confirm(&quot;Are you sure you want to delete?&quot;)) { return true; } return false;'><i class='fa fa-trash-o'></i></a>";
										}
									?>
								</td>
							</tr>
...
<script>
	...
	jQuery(document).ready(function(){
		...
		jQuery(document).on("click",".testreport",function() {
			...
			jQuery("#reportContent").load("<?php echo WWW_BASE; ?>admin/darkadaptations/view/"+testreportId+ "?" + new Date().getTime()+ new Date().getMilliseconds(), function(result) {
