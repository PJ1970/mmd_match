...
<div class="content">
	<div class="">
		<div class="page-header-title">
			<h4 class="page-title">PUP Test Report</h4>
		</div>
	</div>
					...
					<div class="panel panel-primary">
						<div class="panel-body">
							...
							<div class="col-md-12 form-group">
								<?php echo $this->Form->create('PupTest', array('type' => 'get', 'url' => array('controller' => 'pup', 'action' => 'pup_list'))); ?>
									...
									<table id="datatable_report1" class="table table-striped table-bordered">
										<thead>
											<tr>
												...
												<th><?php echo $this->Paginator->sort('PupTest.created', 'Test Date'); ?> </th>
												<?php if ($Admin['user_type'] == 'Admin') { ?>
													<th>Office Name</th>
													<th><?php echo $this->Paginator->sort('PupTest.staff_name', 'Staff User'); ?> </th>
												<?php } ?>
												<th><?php echo $this->Paginator->sort('PupTest.patient_name', 'Patient name'); ?> </th>
											...
											foreach ($datas as $key => $data) { ?>
												...
												<tr>
													...
													<?php if ($Admin['user_type'] == 'Admin') { ?>
														...
														<td>
															<a style='cursor: pointer;' title='View' staffId='<?php echo @$data['PupTest']['staff_id'] ?>' class='staff loaderAjax'><?php echo @$data['User']['first_name'] . ' ' . @$data['User']['middle_name'] . ' ' . @$data['User']['last_name']; ?></a>
														</td>
													<?php } ?>
													<td>
														...
														<a style='cursor: pointer;' title='View' patientId='<?php echo @$data['PupTest']['patient_id'] ?>' class='patient loaderAjax'><?php echo @$data['Patient']['first_name'].' '.$data['Patient']['last_name'] ?></a>
													</td>
													...
													<td class="action_sec">
														...
														?>
														<?php
														echo "<a style='cursor: pointer;'  title='View' testreportId='" . $data['PupTest']['id'] . "' class='testreport loaderAjax' ><i class='fa fa-eye' aria-hidden='true'></i></a>";
														?>
														...
														if (!empty($Admin) && ($Admin['user_type'] == "Subadmin" || $Admin['user_type'] == "Admin")) {
															echo "<a href='".WWW_BASE."admin/pup/delete/" . $data['PupTest']['id'] . "' title='Delete' onclick='if (confirm(&quot;Are you sure you want to delete?&quot;)) { return true; } return false;'><i class='fa fa-trash-o'></i></a>";
														}
														?>
													</td>
												</tr>
...
<script>
	...
	jQuery(document).ready(function () {
		...
		jQuery(document).on("click", ".testreport", function () {
			...
			jQuery("#reportContent").load("<?php echo WWW_BASE; ?>admin/pup/view/" + testreportId + "?" + new Date().getTime() + new Date().getMilliseconds(), function (result) {
