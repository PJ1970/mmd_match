...
<div class="content">
      <div class="">
        <div class="page-header-title">
          <h4 class="page-title">STB Test Report</h4>
        </div>
					...
					<?php
						...
						<?php $Admin = $this->Session->read('Auth.Admin');?>
							...
							<div class="col-md-12 form-group">
								<?php echo $this->Form->create('StbTest',array('type' => 'get','url' => array('controller' => 'Stb','action' => 'stb_list'))); ?>
					  ...
					  <div class="row">
						<div class="table-responsive col-md-12 col-sm-12 col-xs-12">
						  <table id="datatable_report1" class="table table-striped table-bordered">
							...
							<!--	<th> <?php echo $this->Paginator->sort('StbTest.created','Date'); ?> </th>-->
							<th> <?php echo $this->Paginator->sort('StbTest.created','Test Date'); ?> </th>
								 <?php if($Admin['user_type']=='Admin'){ ?><th>Office Name</th>
								<th>  <?php echo $this->Paginator->sort('StbTest.staff_name','Staff User'); ?> </th><?php } ?>
								 
								<th>  <?php echo $this->Paginator->sort('StbTest.patient_name','Patient name'); ?> </th>
							...
							<?php $ptdata_id=$data['StbTest']['id'];  ?>
							<tr>
								<td data-order="<?php echo $data['StbTest']['id']; ?>"><?php echo $key+1; ?></td>
							<!--	<td><?php echo date('d M Y h:i:s a',strtotime($data['StbTest']['created'])); ?></td>-->
								<td><?php echo date('d M Y h:i:s a',strtotime($data['StbTest']['created'])); ?></td>
							     <?php if($Admin['user_type']=='Admin'){ ?><td><?php echo @$data['Office']['name']; ?></td>
								<td>
									<a style='cursor: pointer;'  title='View' staffId='<?php echo @$data['StbTest']['staff_id'] ?>' class='staff loaderAjax' ><?php echo @$data['User']['first_name'].' '.@$data['User']['middle_name'].' '.@$data['User']['last_name']; ?></a>
								</td><?php } ?>
								 
								<td>
									<!-- <a style='cursor: pointer;'  title='View' patientId='<?php echo @$data['StbTest']['patient_id'] ?>'class='patient loaderAjax'><?php echo @$data['StbTest']['patient_name'] ?></a> -->
									<a style='cursor: pointer;'  title='View' patientId='<?php echo @$data['StbTest']['patient_id'] ?>'class='patient loaderAjax'><?php echo @$data['Patient']['first_name'].''.$data['Patient']['middle_name'].''.$data['Patient']['last_name'] ; ?></a>
								</td>
								...
								<td class="action_sec"> 
							     <?php 
									...
									<?php
										echo "<a style='cursor: pointer;'  title='View' testreportId='".$data['StbTest']['id']."' class='testreport loaderAjax' ><i class='fa fa-eye' aria-hidden='true'></i></a>";
									?>
									...
									<a title="View pdf report" href="<?php echo WWW_BASE.'uploads/stbdata/'.$data['StbTest']['file']; ?>" target="_blank"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>
									...
									<!-- <a title="Download Dicom Report" href="<?php echo $this->Html->url(['controller'=>'unityreports','action'=>'exportDicomda',$data['StbTest']['patient_id'], $data['StbTest']['file']]); ?>" target="_blank"><i class="fa fa-image" aria-hidden="true"></i></a>-->
									 	&nbsp;&nbsp;&nbsp;
									 		<a  title='Download DICOM Report' href="<?php echo $this->Html->url(['controller'=>'unityreports','action'=>'exportImageStb',$data['StbTest']['file']]); ?>" target="_blank"><i class="fa fa-image" aria-hidden="true"></i></a>
									<?php //pr($Admin); die;
										if(!empty($Admin) && ($Admin['user_type'] == "Subadmin" || $Admin['user_type'] == "Admin")){
											echo "<a href='/admin/stb/delete/".$data['StbTest']['id']."' title='Delete' onclick='if (confirm(&quot;Are you sure you want to delete?&quot;)) { return true; } return false;'><i class='fa fa-trash-o'></i></a>";
										}
									?>
								</td>
							</tr>
							<?php }
							  if(isset($this->params['paging']['StbTest']['pageCount'])){ ?>
								<tr> 
...
<script>
	...
	jQuery(document).ready(function(){
		...
		jQuery(document).on("click",".testreport",function() {
			...
			jQuery("#reportContent").load("<?php echo WWW_BASE; ?>admin/stb/view/"+testreportId+ "?" + new Date().getTime()+ new Date().getMilliseconds(), function(result) {
