...
<div class="content">
	<div class="">
		<div class="page-header-title">
			<h4 class="page-title">VT Test Report</h4>
		</div>
	</div>
					...
					<div class="panel panel-primary">
						<div class="panel-body">
							...
							<div class="col-md-12 form-group">
								<?php echo $this->Form->create('VtTest', array('type' => 'get', 'url' => array('controller' => 'vt', 'action' => 'vt_list'))); ?>
									...
									<table id="datatable_report1" class="table table-striped table-bordered">
										<thead>
											<tr>
												...
												<th><?php echo $this->Paginator->sort('vtTest.created', 'Test Date'); ?> </th>
												<?php if ($Admin['user_type'] == 'Admin') { ?>
													<th>Office Name</th>
													<th><?php echo $this->Paginator->sort('vtTest.staff_name', 'Staff User'); ?> </th>
												<?php } ?>
												<th><?php echo $this->Paginator->sort('vtTest.patient_name', 'Patient name'); ?> </th>
										...
										<?php if (!empty($datas)) {
											foreach ($datas as $key => $data) { ?>
												<?php $ptdata_id = $data['VtTest']['id']; ?>
												<tr>
													<td data-order="<?php echo $data['VtTest']['id']; ?>"><?php echo $key + 1; ?></td>
													<td><?php echo date('d M Y h:i:s a', strtotime($data['VtTest']['created'])); ?></td>
													<?php if ($Admin['user_type'] == 'Admin') { ?>
														<td><?php echo @$data['Office']['name']; ?></td>
														<td>
															<a style='cursor: pointer;' title='View' staffId='<?php echo @$data['VtTest']['staff_id'] ?>' class='staff loaderAjax'><?php echo @$data['User']['first_name'] . ' ' . @$data['User']['middle_name'] . ' ' . @$data['User']['last_name']; ?></a>
														</td>
													<?php } ?>
													<td>
														<!-- <a style='cursor: pointer;' title='View' patientId='<?php echo @$data['VtTest']['patient_id'] ?>' class='patient loaderAjax'><?php echo @$data['VtTest']['patient_name'] ?></a> -->
														<a style='cursor: pointer;' title='View' patientId='<?php echo @$data['VtTest']['patient_id'] ?>' class='patient loaderAjax'><?php echo @$data['Patient']['first_name'].''.$data['Patient']['middle_name'].' '.$data['Patient']['last_name'] ; ?></a>
													</td>
													...
													<td class="action_sec">
														...
														?>
														<?php
														echo "<a style='cursor: pointer;'  title='View' testreportId='" . $data['VtTest']['id'] . "' class='testreport loaderAjax' ><i class='fa fa-eye' aria-hidden='true'></i></a>";
														?>
														...
														<?php if(!empty($data['VtTest']['file'])){ ?>
															<a title="View pdf report" href="<?php echo WWW_BASE . 'app/webroot/VtTestControllerData/' . $data['VtTest']['file']; ?>" target="_blank"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>
														<?php } ?>
														...
														if (!empty($Admin) && ($Admin['user_type'] == "Subadmin" || $Admin['user_type'] == "Admin")) {
															echo "<a href='".WWW_BASE."admin/vt/delete/" . $data['VtTest']['id'] . "' title='Delete' onclick='if (confirm(&quot;Are you sure you want to delete?&quot;)) { return true; } return false;'><i class='fa fa-trash-o'></i></a>";
														}
														?>
													</td>
												</tr>
											<?php }
											if (isset($this->params['paging']['VtTest']['pageCount'])) { ?>
												<tr>
...
<script>
	...
	jQuery(document).ready(function () {
		...
		jQuery(document).on("click", ".testreport", function () {
			...
			jQuery("#reportContent").load("<?php echo WWW_BASE; ?>admin/vt/view/" + testreportId + "?" + new Date().getTime() + new Date().getMilliseconds(), function (result) {
