...
class VtController extends AppController
{
	public $uses = array('User','VtTest', 'Patients', 'Tests', 'Practice', 'Office', 'Files', 'VfPointdata', 'Pointdata', 'Masterdata', 'MasterPointdata', 'DevicePreference', 'Diagnosis', 'Cms', 'DarkAdaption', 'DaPointData', 'ActTest', 'ActPointdata');
	...
	/*Test Report List*/
	public function admin_vt_list()
	{
		$Admin = $this->Auth->user();
		$conditions = array('VtTest.is_delete' => '0');
		if (!empty($this->Session->read('Search.office'))) {
			$office_id = $this->Session->read('Search.office');
			$conditions['OR'][] = array('Lower(VtTest.office_id)' => $office_id);
		} else {
			if ($Admin['user_type'] == 'Subadmin' || $Admin['user_type'] == 'Staffuser') {
				$conditions['AND'][] = array('Lower(VtTest.office_id)' => $Admin['office_id']);
			}
		}
		if (!empty($this->request->query['patent_id'])) {
			$conditions['AND'][] = array('Lower(VtTest.patient_id)' => $this->request->query['patent_id']);
		}
		if (!empty($this->request->query['search'])) {
			$staff_name = explode(" ", @$this->request->query['search']);
			$search = strtolower(trim($this->request->query['search']));
			$conditions['OR'][] = array('Lower(VtTest.patient_name) like' => '%' . $search . '%');
		...
		$this->paginate = array('VtTest' => $params);
		$datas = $this->paginate('VtTest');
		//pr($datas); die;
		if ($Admin['user_type'] == 'Subadmin' || $Admin['user_type'] == 'Staffuser') {
	...
	public function admin_export($id = null)
	{
		...
		if (!empty($id)) {
			$data = $this->VtTest->findById($id);
			$file_name = str_replace(" ", "_", $data['VtTest']['patient_name']) . "_" . date('YmdHis', strtotime($data['VtTest']['created']));
	...
	public function admin_delete($id = NULL)
	{
		...
		if ($Admin['user_type'] == 'Admin' || $Admin['user_type'] == 'Subadmin') {
			$delete_record = $this->VtTest->updateAll(
				array('VtTest.is_delete' => '1'),
				array('VtTest.id' => $id)
			);
			if ($delete_record) {
				$this->Session->setFlash("VT Test deleted successfully.", 'message', array('class' => 'message'));
			} else {
