...
{
	public $uses = array('Admin', 'User', 'Patient', 'Office', 'Officereport', 'NewUserDevice', 'PatientFiles', 'Pointdata', 'PushNotificationLog', 'DarkAdaption', 'TestStart', 'TestDevice', 'UserDefault', 'DeviceMessage', 'Masterdata', 'OfficeReportBackup', 'DeviceMessage', 'PatientVideoViews', 'Video','PatientDiagnosis','Diagnosis');
	...
	public $allowedActions = array('admin_cleardevice', 'admin_checkdevicestatus', 'admin_cleardevicedata','admin_Updatestatus30days');
	...
	public function admin_callapi($value = '')
	{
		...
		$url = "https://www.portal.micromedinc.com/apisnew/PointData";
...
VF|VF_SC1_RESULT|-0.1856|-0.1856|0|10.0", "VF|VF_RESEND_DATA_COMPLETED|23", "VF|VF_FILE_UPLOADED|35045", "VF|VF_READY");
	...
	public function admin_test3()
	{
		...
		if ($check == 1) {
			...
			$officereport = $this->OfficeReportBackup->find('first', array('conditions' => array('OfficeReportBackup.office_id' => $user['Office']['id'])));
...
//print_r($data);
		...
		if (isset($data['Pointdata'])) {
			...
			if ($this->OfficeReportBackup->save($officereport)) {
				...
				if ($checkradio == 'dicome') {
					...
					$file_url = 'https://www.portal.micromedinc.com/pointData/' . $fileName;
	...
	public function admin_start_test($id = null, $testId = 0, $strategy = 0, $testname = '')
	{
		...
		$user = $this->Session->read('Auth.Admin');
		$patient_previous_test = $this->Pointdata->find('first', array('recursive' => -1,
            'conditions' => array('Pointdata.patient_id' => $id,'Pointdata.test_name' => $vfTestNames)));   
		if ($user['office_id'] == 0) {
	    ...
	    $language_data = $this->Officelanguage->find('all',array('recursive' => 1, 'conditions' =>  array('Officelanguage.office_id' =>$user['office_id'])));
	    $language_datas = array();
	   
        foreach($language_data as $key => $value){
             $value['Language']['language_id'] = $value['Language']['l_id'];
             unset($value['Language']['l_id']);
             unset($value['Language']['code']);
             unset($value['Language']['id']);
             unset($value['Language']['is_delete']);
             unset($value['Language']['created']);
             unset($value['Language']['modified']);
             $language_datas[]=$value['Language'];
        }
        if(empty($language_datas)){
            $language_datas[0]['name']='English';
            $language_datas[0]['language_id']=1;
            
            $language_datas[1]['name']='Spanish';
            $language_datas[1]['language_id']=2;
            $language_datas[2]['name']='French';
            $language_datas[2]['language_id']=3;
            $language_datas[3]['name']='Portuguese';
            $language_datas[3]['language_id']=4;
            
            $language_datas[4]['name']='Arabic';
            $language_datas[4]['language_id']=5;
            $language_datas[5]['name']='Hindi';
            $language_datas[5]['language_id']=6;
            $language_datas[6]['name']='Chinese';
            $language_datas[6]['language_id']=7;
            
            $language_datas[7]['name']='Vietnamese';
            $language_datas[7]['language_id']=8;
            $language_datas[8]['name']='Chinese-Cantonese';
            $language_datas[8]['language_id']=9; 
        }
		if (!empty($user_default)) {
		 ...
		 $patient_previous_test =  $patient_previous_test = empty($patient_previous_test)?1:0;
		 
		$this->set(compact(['data', 'test_device', 'user_default', 'user_preset', 'testData', 'defoult_device','language_datas','patient_previous_test']));
	}
...
public function admin_start_test3($id = null, $testId = 0, $strategy = 0, $testname = '')
	...
	public function admin_start_test_vs($id = null, $testId = 3, $strategy = '', $testname = '')
	{
		...
		$this->set(compact(['data', 'test_device', 'user_default', 'user_preset', 'testData', 'defoult_device','language_datas']));
	...
	public function admin_start_test_fdt($id = null, $testId = 0, $strategy = 0, $testname = '')
	{
		...
		$this->loadModel('UserPreset');
		$patient_previous_test = $this->Pointdata->find('first', array('recursive' => -1,
            'conditions' => array('Pointdata.patient_id' => $id,'Pointdata.test_name' => $fdtTestNames))); 
		$this->Patient->unbindModel(
			array('hasMany' => array('Pointdata'))
		);
		$data = $this->Patient->find('first', array('recursive' => 2, 'conditions' => array('Patient.id' => $id)));
	...
	public function admin_findtest($patent_id = null)
	{
		if ($patent_id == null) {
			...
			if ($da_data > $point_data) {
				...
				$test_page_name = 'start_test'; //da link
	...
	public function admin_addPatient(){
		...
		$conditions['Diagnosis.type'] = 0;
		$diagnosis = $this->Diagnosis->find('list',array('fields'=>array('id','name'),'conditions' =>  $conditions));

		$conditions['Diagnosis.type'] = 1;
		$diagnosis_neuro = $this->Diagnosis->find('list',array('fields'=>array('id','name'),'conditions' =>  $conditions));
		if(!empty($TestDevice)){
			$TestDeviceslist = $TestDevice;		
		}
		$this->set(compact('TestTypethresholdArray','TestDeviceslist','diagnosis','patientDiagnosis','diagnosis_neuro'));
		if ($this->request->is('post')) { 
			...
			/*if(!empty($this->request->data['Patient']['dob'])) {
				$this->request->data['Patient']['dob']= date ("Y-m-d", strtotime($this->request->data['Patient']['dob']));
			}else{
				$this->request->data['Patient']['dob']='';
			}*/
			date_default_timezone_set('UTC');
			...
			if ($this->Patient->save($this->request->data, array('validate' => true))) {
				...
				$lastId = $this->Patient->id;
				foreach($this->request->data['PatientDiagnosi'] as $value){
						if(!empty($value['patient_iagnosis']) && $value['patient_iagnosis']!=0 ){
							$pdata['PatientDiagnosis']['patient_id'] = @$lastId;
							$pdata['PatientDiagnosis']['diagnosis_id'] = $value['patient_iagnosis']; 
							//pr($pdata);die;
							$this->PatientDiagnosis->create();
							$result_p = $this->PatientDiagnosis->save($pdata);
						}
				}
        	    ...
        	    if($this->request->data['setup_test']==1){
				$test_name=array();
                $test_name[14]['name']='Visual Field Test';//'VF';
                $test_name[14]['path']='start_test';
                $test_name[15]['name']='FDT';
                $test_name[15]['path']='start_test_fdt';
                //$test_name[23]['name']='Dark Adaptation	';//'DA';
                //$test_name[23]['path']='start_test_da';
                $test_name[25]['name']='Vision Screening';//'VS';
                $test_name[25]['path']='start_test_vs';
                //$test_name[34]['name']='Pupilometer';//'PUP';
                //$test_name[34]['path']='start_test_pup';
        		$officereport = $this->Officereport->find('all', array('recursive' => -1,'conditions' => array('Officereport.office_id' => $Admin['Office']['id'],'Officereport.office_report'=>array(14,15,25))));  // 23,34
                
                if(count($officereport) ==1){
                    $this->redirect(array('controller' => 'patients', 'action' => $test_name[$officereport[0]['Officereport']['office_report']]['path'], $lastId));
                }else if(count($officereport) >1){ 
                    $this->Session->write('lastId', $lastId); 
                    $this->redirect(array('controller' => 'patients', 'action' => 'admin_addPatient'));
                }
        	   }
				$this->redirect(array('controller' => 'patients', 'action' => 'admin_patients_listing'));
			}else{
			    if(@$this->request->query['rempve_layout']==1){
			        $data['error']  = $this->Patient->validationErrors;
	...
	//This function for start test option
	public function admin_startTestOption($lastId){
	   
	       $this->layout = false;
	       $Admin = $this->Session->read('Auth.Admin');
	      	$test_name=array();
            $test_name[14]['name']='Visual Field';//'VF'; 1
            $test_name[14]['path']='start_test';
            $test_name[15]['name']='FDT';  //2
            $test_name[15]['path']='start_test_fdt';
            //$test_name[23]['name']='Dark Adaptation	';//'DA'; 4
            //$test_name[23]['path']='start_test_da';
            $test_name[25]['name']='Vision Screening';//'VS'; 3
            $test_name[25]['path']='start_test_vs';
            //$test_name[34]['name']='Pupil Response Test';//'PUP'; 5
            //$test_name[34]['path']='start_test_pup';
    		$officereport = $this->Officereport->find('all', array('recursive' => -1,'conditions' => array('Officereport.office_id' => $Admin['Office']['id'],'Officereport.office_report'=>array(14,15,25)))); // 23,34
            $order=array(14,15,25);  // 23,34
            $officereports=array();
            foreach($order as $key => $value){
                foreach($officereport as $key2 => $value2){
                    if($value2['Officereport']['office_report']==$value){
                        $officereports[] = $value2;
                    }
                }
            }
            $this->set(compact('test_name','officereports','lastId'));
	}
	...
	public function admin_edit($id = null)
	{
		...
		$patientDiagnosis = $this->PatientDiagnosis->find('list', array('conditions' => array('PatientDiagnosis.patient_id' => $id), 'fields' => array('diagnosis_id','patient_id')));
		$conditions['Diagnosis.is_delete'] = 0;
		$conditions['Diagnosis.type'] = 0;
		$diagnosis = $this->Diagnosis->find('list',array('fields'=>array('id','name'),'conditions' =>  $conditions));
		...
		$this->set(compact('TestTypethresholdArray','TestDeviceslist','patientDiagnosis','diagnosis','diagnosis_neuro'));
		...
		if ($this->request->is(array('post', 'put'))) {
			...
			if ($this->Patient->save($this->request->data)) {
				...
				$lastId = $id;
				$deletedata = $this->PatientDiagnosis->deleteAll(array('PatientDiagnosis.patient_id'=>$lastId),false);
				foreach($this->request->data['PatientDiagnosi'] as $value){
						if(!empty($value['patient_iagnosis']) && $value['patient_iagnosis']!=0 ){
							$pdata['PatientDiagnosis']['patient_id'] = @$lastId;
							$pdata['PatientDiagnosis']['diagnosis_id'] = $value['patient_iagnosis']; 
	...
	public function admin_view($id = null)
	{
		...
		$vs = $this->Pointdata->find('first', ['conditions' => array('Pointdata.patient_id' => $id, 'Pointdata.test_name' => $vsTestNames)]);
		...
		$this->ActTest->unbindModel(array('belongsTo' => array('User', 'Patient'), 'hasMany' => array('ActPointdata')));
		$act = $this->ActTest->find('first', ['conditions' => array('ActTest.patient_id' => $id)]);

		$this->PupTest->unbindModel(array('belongsTo' => array('User', 'Patient'), 'hasMany' => array('PupPointdata')));
		$pup = $this->PupTest->find('first', ['conditions' => array('PupTest.patient_id' => $id)]);

		$this->VtTest->unbindModel(array('belongsTo' => array('User', 'Patient')));
		$vt = $this->VtTest->find('first', ['conditions' => array('VtTest.patient_id' => $id)]);

		$this->VaData->unbindModel(array('belongsTo' => array('User', 'Patient')));
		$va = $this->VaData->find('first', ['conditions' => array('VaData.patient_id' => $id)]);

		$this->StbTest->unbindModel(array('belongsTo' => array('User', 'Patient','Test'), 'hasMany' => array('StbPointdata')));
		$stb = $this->StbTest->find('first', ['conditions' => array('StbTest.patient_id' => $id)]);

		$test['vf'] = (empty($vf)) ? 0 : 1;
		$test['fdt'] = (empty($fdt)) ? 0 : 1;
		$test['vs'] = (empty($vs)) ? 0 : 1;
		$test['pup'] = (empty($pup)) ? 0 : 1;
		$test['da'] = (empty($da)) ? 0 : 1;
		$test['act'] = (empty($act)) ? 0 : 1;
		$test['vt'] = (empty($vt)) ? 0 : 1;
		$test['va'] = (empty($va)) ? 0 : 1;
		$test['stb'] = (empty($stb)) ? 0 : 1;
	...
	/*public function admin_Updatestatus30days(){ 
		...
		foreach($office_list as $office_lists){
			...
			//if(@$archiStatus == 1){ 
				...
				$conditions['Patient.created_at_for_archive <']=$last_date_start; 
				$this->Patient->updateAll(array('Patient.status' => 0,'Patient.archived_date' => "'".$data_archived_date."'",'Patient.created' => "'".$data_archived_date."'"), array($conditions));
			//}
		}
		exit();
		die();
	...
	public function admin_play_video()
	{
		
		 $this->loadModel('VideoRequest'); 
		$video_request = $this->VideoRequest->find('first', array('conditions' => array('VideoRequest.office_id' => $this->request->data['office_id'], 'VideoRequest.device_id' => $this->request->data['device_id'])));
		 if($this->request->data['type'] ==4 && isset($video_request['VideoRequest']['id'])){
		 	$this->VideoRequest->delete($video_request['VideoRequest']['id']);
		 }else{
		 	$video_request['VideoRequest']['video'] = $this->request->data['video'];
			$video_request['VideoRequest']['office_id'] = $this->request->data['office_id'];
			$video_request['VideoRequest']['device_id'] = $this->request->data['device_id'];
			$video_request['VideoRequest']['patient_id'] = $this->request->data['patient_id'];
			$video_request['VideoRequest']['status'] = $this->request->data['type'];
			$this->VideoRequest->save($video_request);
		 }
	...
	public function admin_view_video()
	{
		...
		$video2 = $this->Video->find('first',array('conditions'=>array('Video.id'=>$video_id))); 
		...
		echo '<video   controls id="video-player"  style=" width: 100%; margin-top: 10px; ">
          <source id="video-player-src"  src="' . WWW_BASE . '/files/video/uploads/' . $Office_folder_name.'/'.$video2["Video"]["video"] . '" type="video/mp4">
        </video>';
	...
	public function admin_autoCompleetList(){
		...
		$this->Patient->virtualFields['patient_name'] = "CONCAT(Patient.first_name, ' ', Patient.last_name)";
		$conditions["Patient.is_delete"] = 0;
		$conditions['Patient.merge_status'] = 0;
		$conditions["Patient.patient_name LIKE"] = '%' . $this->request->query['search'] . "%";
		if ($Admin['user_type'] == 'Admin') {
			//$this->Pointdata->useTable = 'pointdatas_admin';
		}else{
			try {
				$this->Patient->useTable = 'patients_'.$Admin['office_id']; 
				$datas = $this->Patient->find('all',array('conditions' =>$conditions, 'recursive' => -1, 'limit' => 10,'fields'=>array('Patient.id','Patient.patient_name')));
		 
			}catch(Exception $e){
			    pr($e);die;
				$this->Patient->useTable = 'patients';
				$datas = $this->Patient->find('all',array('conditions' =>$conditions, 'recursive' => -1, 'limit' => 10, 'fields'=>array('Patient.id','Patient.first_name','Patient.middle_name','Patient.last_name')));
	...
	//This function for add Patient
	public function admin_addPatientNew(){
